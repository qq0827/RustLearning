...
/// 2. { //函数主体内容 }
/// 简写方式 fn func_test() { rintln!("denfine function!"); }
fn func_test() -> () {
    println!("denfine function!");
}

///# 带返回值的函数
///1. 在函数名后括号中 先写参数名 后跟参数类型. 这和c/c++/c#不同, 和go语言一样
///2. 和其他语言(c/c++/c#/go)不一样, 返回值前不用加return,这点有点怪异,在业务逻辑复杂的情况,看代码不是很好
